...
describe('shoppingItemService', () => {
  ...
  describe('createShoppingItem', () => {
    it('should create a shopping item with valid data including creator', async () => {
      const data = {
        name: 'Milk',
        quantity: 2,
    ...
    it('should throw error if creator field is missing', async () => {
      await expect(shoppingItemService.createShoppingItem({ name: 'Milk', quantity: 2 })).rejects.toThrow('Missing required field: creator');
    });
    it('should throw error for invalid data', async () => {
      prisma.shoppingItem.create.mockRejectedValue(new Error('Invalid data'));
      await expect(shoppingItemService.createShoppingItem({})).rejects.toThrow('Missing required field: creator');
    });
  });
